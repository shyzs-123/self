...
if (typeof Cesium !== 'undefined')
  ...
  if (measure) {
    // 重新计算面积，防止缓存值为空
    let wgs84Points = $this.transformCartesianArrayToWGS84Array(positions)
    area = $this.getPositionsArea(wgs84Points)
    // 抬高标签避免贴地遮挡
    let carto = Cesium.Cartographic.fromCartesian(position)
    let newCart = Cesium.Cartesian3.fromRadians(
      carto.longitude,
      carto.latitude,
      carto.height + 20
    )
    _labelEntity.position = newCart
    _labelEntity.label = {
      text: (area / 1000000.0).toFixed(4) + '平方公里',
      show: true,
      showBackground: true,
      font: '16px monospace',
      horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
